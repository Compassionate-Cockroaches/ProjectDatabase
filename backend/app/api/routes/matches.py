...
from app.schemas.match_player_stats import MatchPlayerStatsResponse, MatchPlayerStatsCreate
...
async def get_match_player_stats(
    ...
    statement = select(
        ...
        Player, MatchPlayerStats.player_id == Player.id
    ).join(
        Team, MatchPlayerStats.team_id == Team.id
    ).where(
...
@router.post("/{match_id}/player-stats", response_model=MatchPlayerStatsResponse, status_code=status.HTTP_201_CREATED)
